...
<p class="section-sub">Who funds whom in Jharkhand. The <b>“Supports in Jharkhand”</b> column links each funder to the implementing org(s) it backs here (gold tags match the Partner directory); where a funder's local grantees aren't public, that's stated plainly. <b>Scale ₹ Cr is organisation-level</b> (annual giving / CSR / fund size, mostly all-India) — <b>not</b> Jharkhand-specific spend, which funders rarely disclose by state; read it as relative magnitude only. Compiled from public sources, kept out of the health scores.</p>
...
function buildExt(){
 ...
 let f='<table><thead><tr><th>Funder</th><th>Supports in Jharkhand</th><th>Scale ₹ Cr <span class="mini">✳ org-level</span></th><th>Ref</th></tr></thead><tbody>';
 EXT_FUND.forEach(o=>{
   ...
   f+='<tr><td><b>'+o.name+'</b></td>'
    ...
    +'<td><b>'+(o.amt||'—')+'</b>'+(o.amtn?'<br><span class="mini">'+o.amtn+'</span>':'')+'</td>'
